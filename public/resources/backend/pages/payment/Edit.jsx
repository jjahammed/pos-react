...
const [inputValue, setInputValue] = useState({
  ...
  useEffect(() => {
    ...
    const getPayment = async () => {
      await axios.get(`/api/payment/${trxId}/${invoice}/edit`).then((res) => {
        if(res.data.status === 200){
          ...
          setInputValue({
            ...
            payment_date : res.data.data.payment_date,
            amount : res.data.data.amount,
            payment_type : res.data.data.payment_type,
            month : res.data.data.month,
            year : res.data.data.year ,
            bank_name : res.data.data.bank_name,
            acc_name : res.data.data.acc_name,
            acc_number : res.data.data.acc_number,
            cheque_number : res.data.data.cheque_number,
            cheque_date : res.data.data.tax,
            mob_type : res.data.data.mob_type,
            mob_number : res.data.data.mob_number,
            mob_trxId : res.data.data.mob_trxId,
            note : res.data.data.note,
            image : res.data.data.image
          })
          setPaymentOption(res.data.data.payment_type)
          setSelectedCrediential({
            label : res.data.data.type == 1 || 4 ? res.data.data.customer?.name : res.data.data?.supplier.name,
  ...
  const submitForm = (e) => {
    ...
    axios.post(`/api/payment/${trxId}`,formData).then(res => {
      if(res.data.status === 200){
          ...
          <form className="theme-form" onSubmit={submitForm}>
              <div className="card">
                  ...
                  <div className="card-body">
                          ...
                          <Input type='text' name='invoice' lblText='Invoice' value={inputValue.invoice} error={inputValue.error_log.invoice} onChange={handleInput} placeholder='Invoice' className='form-control' readOnly/>
                        ...
                        <div>
                          ...
                          <div className="col-lg-3 col-12 text-center">
                              <Radio name="payment_type" id="radioinline1" lbl="Cash" defaultValue="1" handleInput={handleInput} defaultChecked={inputValue.payment_type == 1 ? true : false}/>
                          </div>
                          <div className="col-lg-3 col-12 text-center">
                              <Radio name="payment_type" id="radioinline2" lbl="Card / Bank" defaultValue="2" handleInput={handleInput}  defaultChecked={inputValue.payment_type == 2 ? true : false}/>
                          </div>
                          <div className="col-lg-3 col-12 text-center">
                              <Radio name="payment_type" id="radioinline3" lbl="Bkash/Nogod" defaultValue="3" handleInput={handleInput}  defaultChecked={inputValue.payment_type == 3 ? true : false}/>
                          </div>
                          <div className="col-lg-3 col-12 text-center">
                              <Radio name="payment_type" id="radioinline4" lbl="Cheque" defaultValue="4" handleInput={handleInput}  defaultChecked={inputValue.payment_type == 4 ? true : false}/>
                          </div>
                      </div>
                      ...
                      paymentOption == 2 ? 
                        ...
                        } 
                        {
                          ...
                          <Image lblText='Invoice Copy' images ={image} image={inputValue.image} onChange={onChangeImage} error={inputValue.error_log.image}/>

                          <h6 className='mb-3'>Note (<small className="form-text text-danger text-center">{inputValue.error_log.note}</small>) </h6>
                          <CKEditor name = 'one' data={inputValue.note} editor={ ClassicEditor } onReady={ editor => { editor.name = 'note'} } onChange={ handlEditor } />
