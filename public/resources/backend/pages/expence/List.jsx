...
const List = () => {
  ...
  useEffect(() => {
      ...
      axios.get('/api/expence').then(res => setProduct(res.data.data));
      setLoading(false);
  ...
  const deletelocation = (e,slug) => {
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`/api/expence/${slug}`).then(res => {
      ...
      <div className="col-xl-12 col-md-12 col-sm-12">
        <div className="card">
          <div className="card-header">
            <h5 className='d-inline'>expence List </h5>
            <span className='d-inline badge btn-primary text-light'>{searchData().length}</span>
            <Link to='/admin/expence/new' className='btn btn-primary pull-right'><i className="fa-regular fa-plus text-bold text-light"></i></Link>
          </div>
          <div className='row'>
                  ...
                  return (
                      <tr key={item.id}>
                        ...
                        <td scope="col" className='text-center'>
                            <Link to={`/admin/expence/${item.trxId}/edit`} className='btn btn-outline-success mr-2'><i className="fa-regular fa-pen-to-square"></i></Link>
