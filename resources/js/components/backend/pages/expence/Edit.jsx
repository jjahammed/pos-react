...
const Edit = () => {
    ...
    const [inputValue, setInputValue] = useState({
      ...
      useEffect(() => {
        document.title = 'expence update'
          axios.get(`/api/expence/${trxId}/edit`).then((res) => {
      ...
      const submitForm = (e) => {
        ...
        axios.post(`/api/expence/${trxId}`,formData).then(res => {
          if(res.data.status === 200){
            console.log(res.data.data);
                        ...
                        <div className="col-sm-12">
                            <form className="theme-form" onSubmit={submitForm}>
                                <div className="card">
                                    ...
                                    <Input type='text' name='amount' lblText='expence amount' value={inputValue.amount} error={inputValue.error_log.amount} onChange={handleInput2} className='form-control' />

                                    <Date lblText='expence Date' value={date} setDateFunction={setDateFunction} error={inputValue.error_log.payment_date}  className='form-control'/>
